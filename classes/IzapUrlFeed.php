...
class IzapUrlFeed extends IzapGetFeed {
	...
	function setUrl($url = '') {
		...
		switch ($this->type) {
			case 'youtube':
				...
				$this->feed = ['url' => $this->youtube_api_capture['api_location'] . $this->video_id . '&key=' . $youtube_api_key, 'type' => 'youtube'];
				break;
			case 'vimeo':
				$explode_char = '/';
				...
				$this->feed = ['url' => $this->vimeo_api_capture['api_location'] . $this->video_id.'.php', 'type' => 'vimeo'];
				break;
			case 'dailymotion':
				$explode_char = '/';
				$url_pram = explode($explode_char, $url);
				$this->video_id = sanitise_string(end($url_pram));
				$this->feed = ['url' => $this->dailymotion_api_capture['api_location'] . $this->video_id.'?fields=title,description,thumbnail_url,id,tags', 'type' => 'dailymotion'];
				break;
			default:
				return 103;
				break;
	...
	function capture() {
		...
		$urltocapture =  new IzapCurl($obj->videoThumbnail);
